class Ability
  ...
  def admin_abilities(user)
    ...
    can :manage, :admin_authors
    can :manage, :admin_categories
    can :manage, :admin_publishers
  ...
  def borrow_request_actions(user)
    ...
    can [:read, :update, :destroy], BorrowRequest, user_id: user.id
    ...
    can :create, Review do |review|
      review.book_id.present? &&
        user.borrow_requests
            .where(status: [:approved, :returned, :borrowed])
